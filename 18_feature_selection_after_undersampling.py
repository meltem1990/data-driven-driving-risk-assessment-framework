...
df = pd.read_excel("d:/USveri/US_FEATURES_REDUCED_SPEARMAN_0.9_WITH_LABEL.xlsx")
...
# Özellik seçimi: yalnızca eğitim verisine uygula
model_fs = XGBClassifier(eval_metric='mlogloss', random_state=42)
model_fs.fit(X_train, y_train)

# Önemli özellikleri seç
selector = SelectFromModel(model_fs, threshold=0.005, prefit=True)
...
selected_df.to_excel("D:/USveri/makalerevision/yeniUS_xgboost_selected_features_fcm.xlsx", index=False)

# Seçilen özellik adlarını ayrı dosyaya yaz (başka dosya adı!)
pd.Series(selected_cols).to_excel(
    "D:/USveri/makalerevision/US_xgboost_selected_feature_names_fcm.xlsx",
    index=False,
    header=["Selected Features"]
)

print("\n Özellik seçimi, modelleme ve sonuçların kaydı başarıyla tamamlandı.")
