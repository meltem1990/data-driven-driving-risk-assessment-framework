...
input_files = [
    os.path.join(base_path, "jerk_output_0820_0835_with_preceding_enriched_exactonly_updated.csv"),
    os.path.join(base_path, "jerk_output_0805_0820_with_preceding_enriched_exactonly_updated.csv"),
    os.path.join(base_path, "jerk_output_0750_0805_with_preceding_enriched_exactonly_updated.csv"),
]
...
output_merged = os.path.join(base_path, "vehicle_all_features_vel_acc_diff_ONLY_ALLSEGMENTS.csv")

# Kullanılacak istatistik listesi 
...
# Ana fonksiyon: Tüm araçlar için özellikleri hesapla
def extract_all_features(df, vehicle_col='Vehicle_ID', sort_col='Frame_ID'):
    ...
    for vehicle_id, group in df.groupby(vehicle_col):
        ...
        # Sadece vel_diff ve acc_diff hesaplanıyor
